...
export default function StockCard() {
  ...
  const submitAdjustment = async () => {
    ...
    if (!balance) {
      toast({
        variant: 'destructive',
        title: 'Pilih item/stok yang akan dikoreksi',
      });
      return;
    }
    ...
    if (qty <= 0) {
      toast({
        variant: 'destructive',
        title: 'Qty koreksi harus lebih dari 0',
      });
      return;
    }
    ...
    if (!adjustForm.reason) {
      toast({
        variant: 'destructive',
        title: 'Alasan koreksi wajib dipilih',
      });
      return;
    }
    ...
    if (adjustForm.reason === 'Lainnya' && !adjustForm.notes.trim()) {
      toast({
        variant: 'destructive',
        title: 'Keterangan wajib diisi untuk alasan Lainnya',
      });
      return;
    }
    ...
    try {
      ...
      await createAuditLog({
        ...
        data_after: {
          ...
          estimated_quantity:
            (Number(balance.quantity) || 0) + (isDecrease ? -qty : qty),
        },
      });
  ...
  const openingItems = useMemo(() => {
    return openingForm.item_type === 'material'
      ? materials
      : products;
  ...
  const openingStageOptions =
    openingStagesByType[openingForm.item_type] || [];

  const openingDuplicateBalance = useMemo(() => {
    if (
      !openingForm.item_id ||
      !openingForm.warehouse_id ||
      !openingForm.inventory_status
    ) {
      return null;
    }
    ...
    return balances.find(b => {
      ...
      const status =
        (!b.inventory_status && b.item_type === 'material')
          ? 'RAW_MATERIAL'
          : (b.inventory_status || '');
      ...
      // Jika batch Opening Balance kosong, jangan biarkan recordStockMovement()
      // memilih balance batch lain secara ambigu.
      return true;
    }) || null;
  }, [
    balances,
    ...
    openingForm.warehouse_id,
    openingForm.inventory_status,
    openingForm.batch_number,
  ]);

  const openOpeningBalance = () => {
    setOpeningForm(
      emptyOpening(user?.full_name || user?.email || '')
    );
    setOpeningOpen(true);
  };
  ...
  const submitOpeningBalance = async () => {
    if (user?.role !== 'admin') {
      toast({
        variant: 'destructive',
        title: 'Opening Balance hanya dapat diposting Administrator',
      });
      return;
    }
    ...
    if (!item) {
      toast({
        variant: 'destructive',
        title: 'Pilih item untuk Saldo Awal',
      });
      return;
    }

    if (!openingForm.inventory_status) {
      toast({
        variant: 'destructive',
        title: 'Stage stok wajib dipilih',
      });
      return;
    }
    ...
    if (qty <= 0) {
      toast({
        variant: 'destructive',
        title: 'Qty Saldo Awal harus lebih dari 0',
      });
      return;
    }
    ...
    if (unitCost < 0) {
      toast({
        variant: 'destructive',
        title: 'HPP/Unit tidak boleh negatif',
      });
      return;
    }

    if (!openingForm.operator.trim()) {
      toast({
        variant: 'destructive',
        title: 'Operator wajib diisi',
      });
    ...
    if (!openingForm.notes.trim()) {
      toast({
        variant: 'destructive',
        title: 'Catatan Saldo Awal wajib diisi',
        description: 'Contoh: Stok existing sebelum implementasi LAB PRO.',
      });
      return;
    }
    ...
    if (openingDuplicateBalance) {
      const existingQty = Number(
        openingDuplicateBalance.available_quantity ??
        openingDuplicateBalance.quantity ??
        0
      );

      toast({
        variant: 'destructive',
        title: 'Saldo Awal untuk kombinasi ini sudah memiliki StockBalance',
        description:
          `Stok sistem: ${existingQty} ${openingDuplicateBalance.unit || ''}. ` +
          `Gunakan Koreksi Stok bila stok tersebut memang sudah tercatat.`,
      });
      return;
    }
    ...
    const note = [
      ...
      openingForm.notes.trim(),
      `Operator: ${openingForm.operator.trim()}`,
    ].join(' · ');
    ...
    try {
      const result = await recordStockMovement({
        item_type: openingForm.item_type,
        item_id: item.id,
        item_code: item.code || '',
        item_name: item.name || '',
        batch_id: batch,
        ...
        inventory_status: openingForm.inventory_status,
        quantity_in: qty,
        quantity_out: 0,
        unit: item.unit || 'unit',
        unit_cost: unitCost,
        transaction_type: 'opening_balance',
        transaction_number: openingNumber,
      ...
      await createAuditLog({
        ...
        entity_type: openingForm.item_type === 'product' ? 'Product' : 'Material',
        entity_id: item.id,
        reference_number: openingNumber,
        reason: openingForm.notes.trim(),
        data_before: {
          quantity: 0,
        },
        data_after: {
          ...
          unit: item.unit || 'unit',
          unit_cost: unitCost,
          stock_balance_id: result?.balance_id || '',
        },
      });

      toast({
        title: 'Saldo Awal berhasil diposting',
        description:
          `${openingNumber} · ${item.name} · +${qty} ${item.unit || 'unit'} · HPP ${fmtMoney(unitCost)}`,
      });
      ...
      toast({
        variant: 'destructive',
        title: 'Posting Saldo Awal gagal',
        description: e?.message || 'Terjadi kesalahan',
      });
    } finally {
  ...
  const filtered = useMemo(() => {
    const rows = data.filter(item => {
      ...
      if (
        ...
        item.transaction_type !== filters.transaction_type
      ) return false;

      if (filters.inventory_status) {
        const norm =
          (!item.inventory_status && item.item_type === 'material')
            ? 'RAW_MATERIAL'
      ...
      if (
        ...
        item.warehouse_id !== filters.warehouse_id
      ) return false;
      ...
      if (
        filters.date_from &&
        item.transaction_date?.slice(0, 10) < filters.date_from
      ) return false;

      if (
        filters.date_to &&
        item.transaction_date?.slice(0, 10) > filters.date_to
      ) return false;
    ...
    const result = sorted.map(r => {
      ...
      const delta =
        (Number(r.quantity_in) || 0) -
        (Number(r.quantity_out) || 0);

      running[key] = (running[key] || 0) + delta;

      const unitCost = resolveBalanceUnitCost(r, {
        materialById,
        stageCostIndex,
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Tipe',
      render: row => (
        <span className="text-[10.5px] px-1.5 py-0.5 bg-muted rounded">
          {transactionTypeLabels[row.transaction_type] || row.transaction_type}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: row =>
        row.inventory_status
          ? (
              <span className="text-[10.5px] px-1.5 py-0.5 bg-muted rounded">
                {STAGE_LABEL[row.inventory_status] || row.inventory_status}
              </span>
            )
          : '—',
    },
    ...
    <div className="p-5 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white border border-border rounded-lg p-3 mb-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            <Label className="text-[11px] mb-1">Dari</Label>
            <Input
              type="date"
          ...
          <div>
            <Label className="text-[11px] mb-1">Sampai</Label>
            <Input
              type="date"
      ...
      <FormModal
        ...
        submitting={openingSubmitting}
        submitLabel="Posting Saldo Awal"
        size="lg"
      >
        <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-[11.5px] text-amber-800">
          Digunakan untuk memasukkan stok fisik yang sudah ada sebelum LAB PRO.
          Jika stok sudah tercatat di sistem, gunakan Koreksi Stok.
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-[12.5px] mb-1">Jenis Item *</Label>
            <Select
              value={openingForm.item_type}
              onValueChange={onOpeningItemTypeChange}
            >
              <SelectTrigger className="h-9 text-[13px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="product">Barang / Produk</SelectItem>
                <SelectItem value="material">Bahan</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Stage *</Label>
            <Select
              value={openingForm.inventory_status}
              onValueChange={v =>
                setOpeningForm(f => ({
                  ...f,
                  inventory_status: v,
        ...
        <div>
          <Label className="text-[12.5px] mb-1">
            {openingForm.item_type === 'product' ? 'Barang / Produk' : 'Bahan'} *
          </Label>
          ...
          <SearchableSelect
            ...
            options={openingItems.map(item => ({
              value: item.id,
              label:
                openingForm.item_type === 'product'
                  ? getInventoryDisplayName(
                      item.name || '—',
                      openingForm.inventory_status
                    )
                  : (item.name || '—'),
              keywords: `${item.code || ''} ${item.name || ''} ${item.category_name || ''} ${item.unit || ''}`,
            }))}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <SearchableSelect
              value={openingForm.warehouse_id}
              onValueChange={v =>
                setOpeningForm(f => ({
                  ...f,
                  warehouse_id: v,
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Batch Legacy</Label>
            <Input
              value={openingForm.batch_number}
              onChange={e =>
                setOpeningForm(f => ({
                  ...f,
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Qty Saldo Awal *</Label>
            <NumberInput
          ...
          <div>
            <Label className="text-[12.5px] mb-1">HPP / Unit *</Label>
            <NumberInput
          ...
          <div>
            ...
            <Input
              value={selectedOpeningItem?.unit || ''}
              disabled
        ...
        {openingDuplicateBalance && (
          <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-[11.5px] text-red-700">
            Kombinasi item + stage + gudang
            {openingForm.batch_number.trim() ? ' + batch' : ''}
            {' '}sudah memiliki StockBalance. Gunakan Koreksi Stok agar Opening Balance tidak terposting dua kali.
          </div>
        )}

        {selectedOpeningItem && (
          <div className="rounded-md border bg-muted/20 p-3 text-[12px] grid grid-cols-2 gap-2">
            <div>
              <span className="text-muted-foreground">Item</span>
              <div className="font-medium">
                {selectedOpeningItem.name || '—'}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="font-medium">
                {STAGE_LABEL[openingForm.inventory_status] ||
                  openingForm.inventory_status}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="font-medium">
                {selectedOpeningWarehouse?.name || '—'}
              </div>
            </div>

            <div>
              <span className="text-muted-foreground">Nilai Opening</span>
              <div className="font-semibold tabular-nums">
                {fmtMoney(
                  (Number(openingForm.quantity) || 0) *
                  (Number(openingForm.unit_cost) || 0)
                )}
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <Textarea
            ...
            rows={2}
            placeholder="Contoh: Stok existing sebelum implementasi LAB PRO."
            className="text-[13px]"
          />
        </div>
      </FormModal>
      ...
      <FormModal
        ...
        submitLabel="Posting Koreksi"
        size="lg"
      >
        <div>
          <Label className="text-[12.5px] mb-1">Item / Stok *</Label>
          ...
          <SearchableSelect
            ...
            options={selectableBalances.map(b => {
              ...
              return {
                value: b.id,
                label:
                  `${display} · ${Number(b.available_quantity ?? b.quantity ?? 0)} ${b.unit || ''}` +
                  `${b.batch_number ? ` · ${b.batch_number}` : ''}`,
                keywords:
                  `${b.item_code || ''} ${b.item_name || ''} ${b.batch_number || ''} ` +
                  `${b.warehouse_name || ''} ${STAGE_LABEL[status] || status}`,
              };
            })}
            placeholder="Cari item / batch / stage..."
            className="h-9 text-[13px]"
          />
        </div>

        {selectedBalance && (
          <div className="grid grid-cols-2 gap-2 rounded-md border bg-muted/20 p-3 text-[12px]">
            <div>
              <span className="text-muted-foreground">Stage</span>
              <div className="font-medium">
                {STAGE_LABEL[
                  (!selectedBalance.inventory_status &&
                    selectedBalance.item_type === 'material')
                    ? 'RAW_MATERIAL'
                    : selectedBalance.inventory_status
                ] ||
                  selectedBalance.inventory_status ||
            ...
            <div>
              ...
              <div className="font-semibold tabular-nums">
                {Number(
                  selectedBalance.available_quantity ??
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-[12.5px] mb-1">Jenis Koreksi *</Label>
            <Select
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Qty Koreksi *</Label>
            <NumberInput
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Alasan *</Label>
            <Select
        ...
        <div>
          <Label className="text-[12.5px] mb-1">
            Keterangan {adjustForm.reason === 'Lainnya' ? '*' : ''}
          </Label>
          ...
          <Textarea
            ...
            rows={2}
            placeholder="Contoh: hasil stock opname fisik, 3 botol pecah, salah input qty..."
            className="text-[13px]"
          />
        </div>
      </FormModal>
